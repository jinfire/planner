...
- pytest가 통과하면 사용자 승인을 기다리지 않고 바로 `git commit` + `git push`까지
  알아서 진행한다. 커밋 후에 무엇을 바꿨는지와 pytest 결과를 요약해서 알려준다.
- 여러 기능을 한 번에 작업했다면 하나의 커밋에 몰아넣지 않고, 기능 단위로 나눠서
  각각 커밋한다 (구현은 이어서 계속 진행하고, 커밋은 자연스러운 단위마다 끊어서).
- 예외: pytest가 실패하거나, 기존 데이터/결과와 호환되지 않는 큰 변경(예: CSV
  스키마 변경)이면 먼저 사용자에게 확인받는다.
...
- 기능을 push할 때는 `PROGRESS.md`도 그 상태에 맞게 업데이트해서 **같은 커밋에 포함**한다.
  - `## 한 것`에 새로 완료된 항목 추가
