...
### 30. Advisor 화면 UX 다듬기 (사용자 피드백 여러 라운드)
- `main.py` - `END`를 `"2024-12-31"` 고정값 대신 `datetime.date.today().isoformat()`로
  변경 - 실행할 때마다 자동으로 오늘까지 데이터를 씀. 이 김에 183,141개 배치도
  최신 데이터로 재실행(2시간 38분 소요, NaN 0건, 최대 26.6년 데이터)
- 자산 배분 막대그래프의 축 라벨: `"IEF(미국 중기국채)"`처럼 자산군을 같이
  보여주던 걸 다시 **티커만**으로 되돌림 (`describe_ticker()`는 문장 설명에만 씀)
- `advisor.py` - `explain_recommendation()`(문장이 딱딱하다는 피드백) 삭제,
  대신 두 함수로 분리:
  - `build_intro_message(survival_weight, growth_weight, risk_weight)`: 맨 위에
    한 번만 뜨는 가치관 매칭 한 줄 (기존과 동일하게 최고 가중치 기준 캔 문구)
  - `build_rank_explanation(rec, rank, total_assets)`: **순위 카드마다** 따로
    붙는 한 줄 설명 (기존엔 1순위에만 있었음) - "n순위 {배분} 조합은 {N}년간
    한 번도 바닥나지 않고 {실제 금액}원까지 불어났어요" 식, 고갈된 경우는
    고갈 시점으로 대체
- **Score 가중치 UI 재설계**: 여러 라운드 피드백을 거침
  - 0~100/0~50로 제각각이던 슬라이더 범위를 **1~10으로 통일** (main.py
    상수를 10으로 나눈 값 - `retirement_score()`가 가중합이라 전부 같은
    배수로 스케일해도 실제 순위는 안 바뀜, UI 표시만 바뀜)
  - "고갈 지연" 슬라이더는 "이미 둘 다 고갈된 것들끼리만" 작동하는 미세
    타이브레이커라 체감 차이가 없어서 슬라이더 제거, `main.SCORE_LONGEVITY_WEIGHT`
    고정값으로 대체
  - 용어: "생존확률" → "성공 확률" → 최종 "**자산 유지**"(가중치 슬라이더),
    "낙폭 방어" → "리스크" → 최종 "**변동성**"(MDD는 계속 병기)
  - **가중치(입력)와 결과 지표(출력)에 똑같은 단어를 쓰면 헷갈린다는 피드백**
    으로 결과 카드 쪽 지표는 완전히 다른 단어로: "자산 고갈률"(=1-생존확률,
    0%가 나오면 안 망했다는 뜻), "연 수익률"(=CAGR), "최대낙폭"(=MDD) - 슬라이더
    쪽 "자산 유지"/"성장"/"변동성"과 헷갈리지 않게 의도적으로 다르게 표현
  - "최종 자산"을 배율("2.38배") 대신 **실제 원화 금액**(`total_assets ×
    final_value`)으로 표시
  - **"성장 가중치를 높이면 결국 변동성(MDD)도 그만큼 커지는 거 아니냐,
    그럼 변동성 슬라이더가 의미 없는 거 아니냐"는 질문에 실측으로 답변**:
    실제 생존 조합들끼리 CAGR-MDD 상관계수는 -0.46(완전 -1.0 아님).
    CAGR이 거의 같은(7.1~7.5%) 조합들만 모아 비교하면 MDD가 -27%~-94%까지
    벌어짐(분산투자 효과) - 그래서 변동성 가중치는 성장 가중치와 중복이
    아니라 독립적으로 의미가 있다는 결론, 슬라이더 유지
  - **"성공확률 100%로 주면 100%짜리만 나오는 거 아니냐(필터 아니냐)"는
    질문**: 필터 아니고 가중합의 계수일 뿐이라고 수식으로 설명. 다만 지금
    데이터셋(2000년~, 저인출률 위주)에서는 분산된 조합 대부분이 자연스럽게
    100% 생존까지 같이 달성해서, 실제로 100% 미만이 상위에 뜨는 걸
    재현하긴 어려웠음(정직하게 사용자에게 공유)
- **차트 확대/축소 비활성화**: `st.line_chart`/`st.bar_chart`가 내부적으로
  `.interactive()`가 켜진 Altair 차트를 씀(스크롤 줌 + 드래그 팬) - 막대그래프도
  꺾은선그래프도 `alt.Chart(...).mark_line()/mark_bar()`로 직접 만들어서
  `.interactive()`를 안 붙이는 방식으로 교체. `requirements.txt`에
  `altair==6.2.2` 명시 추가
- 자산 가치 추이 그래프: 배율(초기자본=1.0) 대신 `total_assets`로 환산한
  **실제 금액**을 **천만원 단위**로 표시(캡션에 단위 표기), y축이 그제서야
  읽을 수 있는 숫자가 됨
- `st.altair_chart(..., use_container_width=True)`가 deprecated라는 로그
  경고를 보고 `width='stretch'`로 교체
- **디자인(색상 테마, 카드 레이아웃 등)은 사용자 확인 후 보류** - 기능/문구부터
  다 다듬고 나중에 투자하기로 함 (다음 후보 8번 참고)
- 이 라운드는 사용자가 "내가 좋다고 할 때까지 계속 수정할 거니까 커밋하지
  마"라고 요청해서 커밋을 미뤄뒀다가, 배치 재실행 확인 후 "다 하고 커밋하고
  푸시하자"는 승인을 받고 커밋 완료(`04d78db`)

### 31. Advisor 화면 비주얼 디자인
- 그동안 미뤄뒀던 디자인(30번 "다음 후보" 8번 참고)을 실제로 진행
- `.streamlit/config.toml` (신규) - 테마 색상 지정 (`primaryColor` 에메랄드
  계열 `#0E9F6E`, 배경/보조배경/텍스트 색상). Streamlit 공식 테마 설정이라
  버튼/슬라이더/포커스 색상에 전체적으로 자동 반영됨
- **자산 배분 막대그래프 → 도넛 차트로 교체** ("막대그래프 너무 구리다"는
  피드백): `alt.Chart(...).mark_arc()`로 구현, 범례에 "IEF 40%"처럼
  티커+비중을 같이 표시(퍼센트 라벨을 도넛 위에 직접 얹는 대신 범례로 - 파이
  조각 위 텍스트 위치 계산이 까다롭고 깨지기 쉬워서 더 안정적인 방식 선택)
- **레이아웃 재구성**: `st.container(border=True)`로 순위별 카드 구분(테두리),
  차트:지표 컬럼 비율을 2:1에서 3:2로 조정해 도넛 차트가 더 크게 보이도록 함,
  순위 제목에 🥇🥈🥉 표시 추가
- 자산 가치 추이 그래프를 선 그래프에서 **그라디언트 채움 영역 차트**(`mark_area`)로
  변경 - 위에서 아래로 진한 초록→흰색 그라디언트
- 페이지 레이아웃을 `centered` → `wide`로 변경(차트가 숨 쉴 공간 확보),
  타이틀에 📊 이모지 추가
- **총 자산 기본값을 5억 → 20억으로 변경**
- 실제 브라우저(DOM 검사로 검증 - 이 세션에서 스크린샷 도구가 일시적으로
  안 돼서 대신 사용): 카드 3개 모두 테두리 적용 확인(`border: 1px solid`),
  도넛 차트가 Set2 팔레트로 3가지 색 구분되어 렌더링됨, 범례에 "BND 40%,
  GLD 40%, QQQ 20%" 등 정상 표시, 총 자산 20억 반영해 최종 자산 금액도
  그만큼 커짐(예: 57억대)
- **버그 발견 및 수정 - "추천 받기 안 눌렀는데 결과가 바뀐다"**: `submitted =
  st.button(...)`는 그 버튼이 눌린 딱 그 rerun에서만 `True`이고, 슬라이더나
  다른 입력을 건드려서 생기는 그 다음 rerun에서는 다시 `False`가 됨.
  `if submitted:` 안에서만 결과를 계산/렌더링하고 있었어서, 결과를 본 다음
  아무 입력이나(가중치 슬라이더, 총 자산 등) 건드리면 `submitted`가
  `False`로 돌아가면서 화면에 있던 결과가 그대로 사라짐(또는 다음 rerun에
  다른 값 기준으로 다시 계산됨) - 사용자가 "안 눌렀는데 왜 변해"라고
  지적해서 발견
  - 수정: `st.session_state`에 계산된 추천 결과와 그때 쓰인 입력값들
    (가중치/총자산/인출률) 스냅샷을 같이 저장. 화면은 항상 이
    session_state에서 읽어서 그리고, "추천 받기"를 실제로 눌렀을 때만
    session_state를 새로 덮어씀 - 다른 위젯을 건드려도 마지막 추천 결과가
    그대로 유지되고, 버튼을 눌러야만 새 값으로 갱신됨
  - 브라우저로 검증: 결과 표시 후 총자산 증가 버튼(추천 재클릭 없이) →
    사이드바 숫자만 바뀌고 카드의 금액은 그대로 유지됨 확인. 이후 "추천
    받기"를 다시 누르니 그제서야 새 총자산 기준으로 갱신됨 확인

### 32. 인출률 8%까지 확장 + 금액 한글(억/만) 표기
- `main.py` - `WITHDRAWAL_RATE_OPTIONS`에 0.065/0.07/0.075/0.08 추가(기존
  2.0~6.0%에서 8.0%까지, 0.5%p 간격 유지) → 인출률 9종에서 13종으로,
  183,141개(20,349 조합 × 9) → 264,537개(20,349 × 13)로 배치 재실행 필요
  (백그라운드로 재실행, 조합 수 44% 증가로 ~3시간 50분 예상)
- `advisor.py` - `format_won(amount)` 추가: `"5,748,496,717원"` 대신
  `"57억 4,849만원"`처럼 억/만 단위로 표기 (만원 미만은 버림 - 헤드라인
  숫자용이지 회계용 아님). `describe_ticker()`처럼 매핑 없는/작은 값은
  안전하게 폴백(억도 만도 없으면 그냥 콤마 숫자)
  - `build_rank_explanation()`의 최종 금액 표기에 적용
  - `advisor/tests/test_advisor.py` - 억+만 조합, 억만 딱 떨어지는 경우,
    억 없이 만 단위만, 만원도 안 되는 소액(폴백) 4가지 검증. 기존
    "최종 금액이 배율 아니라 원화로 나오는지" 테스트도 새 포맷에 맞게 갱신
- `app.py` - 사이드바 총자산/월인출액 캡션, 결과 카드의 "월 인출액"/
  "최종 자산" 지표 전부 `format_won()`으로 통일 (한 화면에 "20억원"과
  "5,748,496,717원"이 섞여 있으면 안 어울려서 전체 통일)
- 실측(기존 results.csv, 4% 그대로 남아있는 조합으로): "57억 4,849만원",
  "666만원" 등 정상 표기 확인. 인출률 드롭다운도 13개 옵션(2.0~8.0%) 확인
- 총 자산 기본값 20억은 이미 반영돼 있어서 그대로 유지
- pytest 115개 통과(기존 111 + 신규 4)
- **배치 재실행 완료** (264,537행, NaN 0건, 13개 인출률 전부 커버). 8.0%로
  실제 추천 확인: TLT 40%/QQQ 20%/VWO 20%/GLD 20% 등 실제로 다른 조합이
  뽑히고(낮은 인출률 때와는 다른 자산군 - MDD가 더 큰 조합들도 등장),
  생존확률 100%, 금액도 "29억 2,484만원"처럼 정상 표기됨
...
8. ~~`app.py` 비주얼 디자인~~ → 31번에서 진행 (테마 색상, 도넛 차트, 카드
   레이아웃, wide 레이아웃)
