...
- 이 라운드 전체는 pytest 통과만 확인하고 **커밋은 미룸** - 사용자가 "내가
  좋다고 할 때까지 계속 수정할 거니까 커밋하지 마"라고 명시적으로 요청해서,
  이번 절(30번) 작업이 다 끝나고 배치 재실행까지 확인된 뒤에야 한 번에 커밋
...
8. `app.py` 비주얼 디자인 - 지금은 Streamlit 기본 테마 그대로라 밋밋함. 사용자
   확인: 기능/문구부터 다 다듬고 나서 나중에 디자인에 시간 투자하기로 함
   (색상 테마 `.streamlit/config.toml`, 카드형 레이아웃 등은 아직 손 안 댐)
