...
1. **Guyton-Klinger 인출 전략** - 인출액 자체를 성과에 맞춰 동적으로 조정
   (상한 가드레일 초과 시 -10%, 하한 미달 시 +10%, 하락한 해 다음엔 물가인상 동결)
2. **롤링 윈도우 테스트** - 시작연도를 하나만 보지 않고 여러 해(예: 1990~2024
   매년)를 다 돌려서 "성공률"로 판단 - 벡터화 덕분에 이제 현실적으로 가능
3. Advisor (LLM) - 1위 포트폴리오가 선정된 이유, 강점/약점/리스크/대안 설명
4. Planner를 실제 API 키로 검증 후 main.py에 연결
5. 버킷 전략용 Monte Carlo 엔진
6. 새 12개 자산으로 고전적 자산배분 전략(60/40 등) 백테스트 돌려보기
